...
- **Public Functions:** Use lower case, underscore separated if more
  than one verb, i.e. `snake_case` (e.g., `check_normality()`,
...
- **Naming**: Common prefix for functions that focus on specific “tasks”
  or workflows (e.g. in package “insight”, `get_*()` to get data,
  `find_*()` to find information, or in package “performance”,
...
- “easystats” uses the argument name `by` to indicate grouping, not
  `group_by` or `at`.
...
- **Description:** A more detailed paragraph explaining the function’s
  purpose.
