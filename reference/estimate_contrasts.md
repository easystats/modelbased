...
- by:
  ...
  interest. For
  [`estimate_means()`](https://easystats.github.io/modelbased/reference/estimate_means.md),
  `by` defaults to `"auto"`, which automatically selects the first focal
  predictor found in the model. If `by = NULL`, the grand mean of the
...
- comparison:
  ...
  - When `backend = "emmeans"`, can be one of `"pairwise"`,
    `"revpairwise"`, `"poly"`, `"consec"`, `"eff"`, `"del.eff"`,
    `"mean_chg"`, `"trt.vs.ctrl"`, `"dunnett"`, `"wtcon"` and some more.
    Defaults to `"revpairwise"`, to return consistent results (regarding
    the sign) with the default `backend = "marginaleffects"`. To test
    multiple hypotheses jointly (usually used for factorial designs),
    `comparison` can also be `"joint"`. See also `method` argument in
...
- estimate:
  ...
  **Conditional Predictions based on artificially constructed reference
  grids**

  - `"specific"`: The estimated means represent a rather "theoretical"
    view of the data. A prediction is made for a specific individual
    from the sample (i.e., a very specific combination of predictor
    values). This answers the question about the expected average value
    of the target variable for a *specific* observation.
...
- post_process:
  ...
  a list of formulas, strings or functions, to process subsequent,
  ...
  post-process tests. See 'Examples'.
...
- iterations:
  ...
  [`estimate_means()`](https://easystats.github.io/modelbased/reference/estimate_means.md),
  [`estimate_slopes()`](https://easystats.github.io/modelbased/reference/estimate_slopes.md),
  or `estimate_contrasts()` (and when `backend = "marginaleffects"`), it
  is passed to the `ndraws` argument of the related `marginaleffects`
  functions, and defines the number of posterior draws to sample from.
  If `NULL` (the default), all draws are used. Note that `ndraws`
  currently only has an effect for Bayesian models fit with **brms** or
  **MCMCglmm**.
...
- To test multiple hypotheses jointly (usually used for factorial
  designs), `comparison` can also be `"joint"`. This option runs
  sequential joint tests on the results of an initial pairwise
  comparison. To test for differences between groups from an global
...
for within- and between effects is described [in this
...
people with identical individual circumstances (such as the same income)
face different opportunities or risks depending on the environment in
which they live. This can be achieved by specifying both numeric
predictors in the `contrast` argument, e.g.
`contrast = c("x_within", "x_between")`. It is also possible to stratify
context effects at different levels of another variable using `by`. If
pairwise comparisons of context effects (i.e., the pairwise comparisons
of the difference of between within- and between-effects, or the
difference of average slopes) at different levels of another variable
are required, add that variable to the `contrast` argument instead, e.g.
`contrast = c("x_within", "x_between", "factor")`. These contrasts can
additionally be stratified by another variable using `by` again, e.g.
`contrast = c("x_within", "x_between", "factor1"), by = "factor2"`. Note
that when average slopes are contrasted, the `comparison` argument has
...
#> ---------------------------------------------------------------------
#> mid    | low    |       2.79 | 2.60 | [-2.31,  7.90] |   1.08 | 0.283
#> high   | low    |       4.76 | 2.97 | [-1.07, 10.59] |   1.60 | 0.110
#> high   | mid    |       1.96 | 2.36 | [-2.67,  6.59] |   0.83 | 0.405
#> 
#> Variable predicted: QoL
#> Predictors contrasted: time
...
#> Level1        | Level2        | Difference |   SE |          95% CI | t(552) |     p
#> ------------------------------------------------------------------------------------
#> low, Group 2  | low, Group 1  |       9.95 | 4.47 | [  1.16, 18.74] |   2.22 | 0.027
#> mid, Group 1  | low, Group 1  |       7.11 | 3.67 | [ -0.10, 14.32] |   1.94 | 0.053
#> mid, Group 2  | low, Group 1  |       8.43 | 3.68 | [  1.20, 15.66] |   2.29 | 0.022
#> high, Group 1 | low, Group 1  |       6.71 | 4.03 | [ -1.20, 14.63] |   1.67 | 0.096
...
#> high, Group 2 | low, Group 2  |       2.80 | 4.36 | [ -5.77, 11.37] |   0.64 | 0.521
#> mid, Group 2  | mid, Group 1  |       1.32 | 2.65 | [ -3.88,  6.51] |   0.50 | 0.619
#> high, Group 1 | mid, Group 1  |      -0.40 | 3.11 | [ -6.51,  5.71] |  -0.13 | 0.898
#> high, Group 2 | mid, Group 1  |       5.64 | 3.53 | [ -1.30, 12.58] |   1.60 | 0.111
#> high, Group 1 | mid, Group 2  |      -1.72 | 3.12 | [ -7.85,  4.42] |  -0.55 | 0.583
#> high, Group 2 | mid, Group 2  |       4.33 | 3.54 | [ -2.63, 11.28] |   1.22 | 0.222
#> high, Group 2 | high, Group 1 |       6.04 | 3.90 | [ -1.62, 13.71] |   1.55 | 0.122
#> 
#> Variable predicted: QoL
#> Predictors contrasted: time
...
#> --------------------------------------------------------------------------
#> mid - low  | Group 1 |       7.11 | 3.67 | [-0.10, 14.32] |   1.94 | 0.053
#> high - low | Group 1 |       6.71 | 4.03 | [-1.20, 14.63] |   1.67 | 0.096
#> high - mid | Group 1 |      -0.40 | 3.11 | [-6.51,  5.71] |  -0.13 | 0.898
#> mid - low  | Group 2 |      -1.53 | 3.68 | [-8.75,  5.70] |  -0.41 | 0.679
#> high - low | Group 2 |       2.80 | 4.36 | [-5.77, 11.37] |   0.64 | 0.521
#> high - mid | Group 2 |       4.33 | 3.54 | [-2.63, 11.28] |   1.22 | 0.222
#> 
#> Variable predicted: QoL
#> Predictors contrasted: time
#> Predictors averaged: time (2)
...
#> Predictors averaged: coffee
#> p-values are uncorrected.
#> Null-hypothesis: Group means equal 15.00
#> 
...
#> ----------------------------------------------------------
#> 1    |      -1.73 | 0.99 | [-3.66,  0.20] | -1.76 |  0.079
#> 2    |      -2.54 | 0.65 | [-3.81, -1.27] | -3.93 | < .001
#> 3    |      -3.36 | 0.97 | [-5.27, -1.45] | -3.45 | < .001
#> 
#> Variable predicted: QoL
...
#> ---------------------------------------------------------------
#> low       |      -1.69 | 1.30 | [-4.24,  0.85] | -1.30 |  0.192
#> mid       |      -3.92 | 0.87 | [-5.64, -2.21] | -4.49 | < .001
#> high      |       1.76 | 1.84 | [-1.84,  5.36] |  0.96 |  0.337
#> 
#> Variable predicted: QoL
#> Predictors contrasted: phq4_within, phq4_between
#> p-values are uncorrected.
...
#> -------------------------------------------------------------------
#> mid    | low    |      -2.23 | 1.57 | [-5.30, 0.84] | -1.42 | 0.154
#> high   | low    |       3.46 | 2.25 | [-0.95, 7.86] |  1.54 | 0.124
#> high   | mid    |       5.69 | 2.03 | [ 1.70, 9.67] |  2.80 | 0.005
#> 
#> Variable predicted: QoL
#> Predictors contrasted: phq4_within, phq4_between
#> p-values are uncorrected.
...
#> --------------------------------------------------------------------
#> mid - low, 2 - 1 |       2.79 | 1.90 | [-0.94,  6.52] | 1.46 | 0.143
#> mid - low, 3 - 1 |       5.58 | 3.81 | [-1.89, 13.04] | 1.46 | 0.143
#> 
#> Variable predicted: QoL
#> Predictors contrasted: education=c('low', 'mid')
#> Predictors averaged: ID (1)
...
#> Parameter | Difference |   SE |        95% CI |    z |     p
#> ------------------------------------------------------------
#> Linear    |       1.97 | 1.35 | [-0.67, 4.61] | 1.46 | 0.143
#> 
#> Variable predicted: QoL
#> Predictors contrasted: education=c('low', 'mid')
#> Predictors averaged: ID (1)
