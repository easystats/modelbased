if (require("testthat") && require("modelbased") && require("rstanarm") && require("insight")) {
  test_that("estimate_means", {
    ...
    expect_equal(c(nrow(estim), ncol(estim)), c(3, 4))
    ...
    estim <- estimate_means(model)
    expect_equal(c(nrow(estim), ncol(estim)), c(6, 5))
    ...
    estim <- estimate_means(model)
    expect_equal(c(nrow(estim), ncol(estim)), c(3, 5))
    ...
    expect_equal(c(nrow(estim), ncol(estim)), c(6, 6))
    estim <- estimate_means(model, levels = "Species=c('versicolor', 'setosa')")
    expect_equal(c(nrow(estim), ncol(estim)), c(2, 5))
    ...
    expect_equal(c(nrow(estim), ncol(estim)), c(15, 6))
    ...
    estim <- estimate_means(model)
    expect_equal(c(nrow(estim), ncol(estim)), c(12, 7))
    estim <- estimate_means(model, fixed = "am")
    expect_equal(c(nrow(estim), ncol(estim)), c(6, 7))
    estim <- estimate_means(model, fixed = "gear='5'")
    expect_equal(c(nrow(estim), ncol(estim)), c(4, 7))
    ...
    estim <- estimate_means(model)
    expect_equal(c(nrow(estim), ncol(estim)), c(8, 7))
