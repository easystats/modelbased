...
test_that("estimate_link", {
  ...
  expect_equal(
    nrow(estimate_link(model, include_random = TRUE, preserve_range = FALSE, length = 5)),
    15
  )
  ...
  expect_equal(
    dim(estimate_link(model, include_smooth = FALSE, length = 3, verbose = FALSE)),
    c(3, 5)
  )

  # STAN_GAMM4
  skip_if_not(.Platform$OS.type == "windows")
  model <- suppressWarnings(rstanarm::stan_gamm4(
    Petal.Length ~ Petal.Width + s(Sepal.Length),
    random = ~ (1 | Species),
    data = iris,
    iter = 100,
...
test_that("estimate_expectation - predicting RE works", {
  ...
  expect_equal(
    out$Predicted,
    c(12.20636, 12.06311, 11.2071, 11.62862, 11.2327, 10.58387, 11.20853, 11.12288),
    tolerance = 1e-4
  )
})
...
test_that("estimate_relation - shape", {
  ...
  expect_silent(estimate_prediction(m, by = "cyl", iterations = 15, keep_iterations = 10))
})


test_that("estimate_relation - nestedLogit", {
  ...
  out <- estimate_relation(mnl1, by = "children", verbose = FALSE)
  expect_identical(dim(out), c(6L, 7L))
  expect_named(
    out,
    c("children", "hincome", "Response", "Predictions", "SE", "CI_low", "CI_high")
  )

  out <- estimate_relation(
    mnl1,
    by = "children",
