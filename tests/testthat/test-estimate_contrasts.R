...
test_that("estimate_contrasts - Frequentist, one factor", {
  ...
  estim <- suppressMessages(estimate_contrasts(
    model,
    contrast = "Species=c('versicolor', 'virginica')",
...
test_that("estimate_contrasts - Frequentist, One factor and one continuous", {
  ...
  # expect_identical(dim(estim), c(3L, 9L))
  estim <- estimate_contrasts(
    model,
    contrast = "Petal.Width",
    by = "Species",
    length = 4,
    backend = "marginaleffects"
  )
...
test_that("estimate_contrasts - Frequentist, One factor and one continuous", {
  ...
  estim <- suppressMessages(estimate_contrasts(
    model,
    by = "Petal.Length=c(2.3, 3)",
    backend = "emmeans"
  ))
  expect_identical(dim(estim), c(1L, 9L))
  estim <- suppressMessages(estimate_contrasts(
    model,
    by = "Petal.Length=c(2, 3, 4)",
    backend = "emmeans"
  ))
  expect_identical(dim(estim), c(3L, 9L))
  estim <- estimate_contrasts(
    model,
    contrast = "Petal.Length=c(2.3, 3)",
    backend = "marginaleffects"
  )
  expect_identical(dim(estim), c(1L, 9L))
  expect_named(
...
test_that("estimate_contrasts - Frequentist, Three factors 1", {
  ...
  expect_equal(
    estim$Difference,
    c(6.98333, 11.275, 18.25833, 4.29167, 11.275, 6.98333),
    tolerance = 1e-4
  )
  ...
  expect_named(
    estim,
    c("Level1", "Level2", "gear", "Difference", "SE", "CI_low", "CI_high", "t", "df", "p")
  )
  expect_equal(
    estim$Difference,
    c(
...
test_that("estimate_contrasts - Frequentist, Three factors 2", {
  ...
  out <- marginaleffects::avg_predictions(
    ...
    newdata = datawizard::data_arrange(
      as.data.frame(insight::get_datagrid(
        fit,
...
test_that("estimate_contrasts - Frequentist, duplicated levels", {
  ...
  estim <- suppressMessages(estimate_contrasts(
    model,
    contrast = c("factor1", "factor2", "factor3"),
    by = "all",
    backend = "emmeans"
  ))
...
test_that("estimate_contrasts - Frequentist, GLM", {
  ...
  estim2 <- suppressMessages(estimate_contrasts(
    model,
    predict = "link",
    backend = "emmeans"
  ))
  ...
  estim3 <- suppressWarnings(suppressMessages(estimate_contrasts(
    model,
    backend = "marginaleffects"
  )))
...
test_that("estimate_contrasts - Bayesian", {
  ...
  estim <- suppressMessages(estimate_contrasts(
    model,
    by = c("Species", "Petal.Length_factor='A'"),
    backend = "emmeans"
  ))
...
test_that("estimate_contrasts - p.adjust", {
  ...
  p_none <- suppressMessages(estimate_contrasts(
    model,
    p_adjust = "none",
    backend = "emmeans"
  ))
  p_tuk <- suppressMessages(estimate_contrasts(
    model,
    p_adjust = "tukey",
    backend = "emmeans"
  ))
  ...
  m_spec <- lm(body_mass ~ species * sex, data = penguins)
  out1 <- estimate_contrasts(m_spec, p_adjust = "tukey")
  ...
  expect_equal(out1$p[7], out2$p[6], tolerance = 1e-4)
})
...
test_that("estimate_contrasts - ratios", {
  ...
  expect_identical(dim(estim), c(3L, 9L))
  estim2 <- marginaleffects::avg_predictions(
    model,
    by = "Species",
...
test_that("estimate_contrasts - marginaleffects vs emmeans", {
  ...
  out5 <- suppressMessages(estimate_contrasts(
    model,
    predict = "response",
    backend = "marginaleffects"
  ))
...
test_that("estimate_contrasts - works with slopes", {
  ...
  out2 <- suppressMessages(as.data.frame(emmeans::emtrends(
    fit,
    specs = ~Petal.Length,
    var = "Petal.Length"
  )))
  ...
  expect_equal(
    out3$Slope,
    as.data.frame(out5$emtrends)$Petal.Length.trend,
    tolerance = 1e-3
  )
  expect_equal(
    out4$Difference * -1,
    as.data.frame(out5$contrasts)$estimate,
    tolerance = 1e-3
  )
})
...
test_that("estimate_contrasts - different options for comparison", {
  ...
  expect_named(out, c("Level", "Difference", "CI_low", "CI_high", "SE", "df", "z", "p"))
...
test_that("estimate_contrasts - row order in data grid doesn't matter", {
  ...
  expect_equal(
    out2$Difference,
    c(0.2036, -0.07087, -0.27447, 0.56658, 0.29721, -0.26937),
    tolerance = 1e-4
  )
})
...
test_that("estimate_contrast, slopes with emmeans", {
  ...
  m1 <- glm(outcome ~ var_binom + var_cont, data = dat, family = binomial(link = "logit"))
  ...
  expect_identical(
    as.character(out$Level1),
    c(
      "var_binom 0, var_cont 0",
      "var_binom 0, var_cont 1",
      "var_binom 1, var_cont -1",
      "var_binom 1, var_cont 0",
      "var_binom 1, var_cont 1",
      "var_binom 0, var_cont 1",
      "var_binom 1, var_cont -1",
      "var_binom 1, var_cont 0",
...
test_that("estimate_contrast, filterin in `by` and `contrast`", {
  ...
  expect_equal(
    out$Difference,
    c(-0.77851, 0.12142, 0.89993, 0.87674, 1.97996, 1.10322, 2.69591, 2.59613, -0.09978),
    tolerance = 1e-4
  )
...
test_that("estimate_contrast, don't calculate slopes for integers", {
  ...
  expect_error(estimate_contrasts(m, "hp"), regex = "Please specify")
