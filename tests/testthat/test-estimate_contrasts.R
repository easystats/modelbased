...
test_that("estimate_contrasts", {
  ...
  testthat::expect_equal(c(nrow(estim), ncol(estim)), c(15, 7))
  estim <- estimate_contrasts(model, fixed = "Petal.Length_factor")
  testthat::expect_equal(c(nrow(estim), ncol(estim)), c(6, 8))
  ...
  testthat::expect_equal(c(nrow(estim), ncol(estim)), c(12, 8))
})
