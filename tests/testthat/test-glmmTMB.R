...
test_that("estimate_means - glmmTMB", {
  ...
  estim2 <- as.data.frame(emmeans::emmeans(
    model,
    ~mined,
    component = "zi",
    type = "response"
...
test_that("estimate_contrasts - glmmTMB", {
  ...
  expect_identical(c(estim1$Level1[1], estim1$Level2[1]), c("no", "yes"))

  ## contrasts marginaleffects for zero-inflated model, count component
  estim3 <- suppressMessages(estimate_contrasts(
    model,
    contrast = "mined",
    backend = "marginaleffects"
  ))
  ...
  estim2 <- predict(
    model,
    type = "zprob",
    newdata = insight::get_datagrid(model, "mined")
  )
  expect_identical(dim(estim1), c(1L, 9L))
...
test_that("estimate_slope - glmmTMB", {
  ...
  estim2 <- as.data.frame(emmeans::emtrends(
    model2,
    "mined",
    var = "cover",
    regrid = "response"
  ...
  estim2 <- suppressWarnings(marginaleffects::avg_slopes(
    model2,
    newdata = datagrid,
    variables = "cover",
    by = "mined"
...
test_that("error edge case", {
  ...
  expect_error(estimate_means(m, "cluster"), regex = "You may try using")
})
