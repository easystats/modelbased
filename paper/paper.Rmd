...
As mentioned above, expected (or predicted) values of the outcome may differ, depending on whether _predictions_ or _marginal means_ are calculated - although both are based on the same model and data. How to interpret these numbers? This is confusing - but don't worry, it's even worse!

The `modelbased` package allows to estimate *four different* quantities of predictions, which can be roughly divided into two categories: data grid-based and empirical predictions.

### Data grid-based predictions

`estimate_relation()` and `estimate_means()` represent the characteristics of the focal predictors in a _balanced data grid_, where all combinations of the predictors' levels are present. The focus lies on the _predictors_, where the aim is to look at associations or comparing certain characteristics. This is done either for a _very specific_ observation (`estimate_relation()`), or a _typical_ observation (`estimate_means()`) from the sample.

### Empirical predictions

If the focus lies on the _predicted outcome_, it is preferable to look at the actual, _empirical_ distribution of the focal predictors' characteristics (and not at a balanced distribution via a data grid). This can be achieved by changing the `estimate` argument in `estimate_means()`. Setting `estimate = "average"` calculates the average expected outcome from those observations _from the sample_ defined by the focal predictors.

In some situations, especially when differences in the predicted outcome between certain groups (i.e., when computing contrasts for levels from the focal predictors) are important and causal effects are assumed, it might be useful to consider a hypothetical population that is not actually present in the sample. This is also called _G-computation_, and can be achieved by setting `estimate = "population"`, which is available in `estimate_means()` - but all `estimate` options are also available for `estimate_contrasts()`.
