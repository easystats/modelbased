---
title: "Describe non-linear curves"
output: 
  github_document:
    toc: true
    fig_width: 10.08
...
vignette: >
  %\VignetteIndexEntry{Describe non-linear curves}
...
This vignette will present how to describe non-linear curvess using `estimate`. Warning: we will go **full Bayesian**. If you're not familiar with the Bayesian framework, we recommend starting with [**this gentle introduction**](https://easystats.github.io/bayestestR/articles/bayestestR.html).
...
# The data: Effect of time on memory
...
Unsuprisingly, the forgetting curve appears to be non-linear, as supported by the litterature suggestiong a 2nd order polynomial curve [@Averell_2011].


# Modelling non-linear curves

We can fit a Bayesian linear mixed regression to model such relationship, adding it a few other variables that could influence this curve, such as the **familiarity with the characters of the movie**, the **language of the movie**, the **immersion** (2D/3D) and the **Sex**.

```{r message=FALSE, warning=FALSE, eval=FALSE}
library(rstanarm)

model <- stan_lmer(Memory ~ poly(Delay, 2) * Characters_Familiarity + (1|Movie_Language) + (1|Immersion) + (1|Sex), data = df)
```
```{r message=FALSE, warning=FALSE, echo=FALSE}
library(rstanarm)

model <- stan_lmer(Memory ~ poly(Delay, 2) * Characters_Familiarity + (1|Movie_Language) + (1|Immersion) + (1|Sex), data = df, chains = 1, iter = 1000, refresh = 0)
```

We can visualise the link between the Delay and the Memory score by using the `estimate_link`.
...
`estimate_smooth` will decompose this curve into linear parts, returning their size (the percentage of the curve of the segment), and the trend (positive or negative). We can now say 
