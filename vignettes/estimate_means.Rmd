...
Another way of analyzing the means is to actually statistically **model them**, rather than simply describe them as they appear in the data. For instance, we could fit a simple Bayesian linear regression modelling the relationship between `Species` and `Sepal.Width`.
...
ggplot(iris, aes(x = Species, y = Sepal.Width, fill = Species)) +
  ...
  geom_line(data = means, aes(y = Median, group = 1), size = 1, alpha = 0.25) +
  geom_pointrange(data = means, aes(y = Median, ymin = CI_low, ymax = CI_high), size = 1, color = "white") +
  
  geom_line(data = means_complex, aes(y = Median, group = 1), size = 1) +
  geom_pointrange(data = means_complex, aes(y = Median, ymin = CI_low, ymax = CI_high), size = 1, color = "yellow") +
  theme_modern()
```
