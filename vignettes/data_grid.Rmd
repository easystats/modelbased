...
```{r message=FALSE, warning=FALSE}
newdata <- data_grid(iris, target = "Sepal.Length", length = 5)
newdata
```
...
```{r message=FALSE, warning=FALSE}
newdata <- data_grid(iris, target = "Sepal.Length", numerics = "min")
newdata
```
...
newdata <- iris %>% 
  ...
  data_grid("Petal.Width", length=5, numerics = "combination")
```
...
<!-- equally spaced values of a "target" variable, and all other variables “fixed” by their mean, median or reference level. The refdata of the psycho package was built to do just that. -->