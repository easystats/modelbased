...
One floating assumption that can be encountered is that "complex models" should be used for "complex" stuff (data, designs, ...) and simple models are enough for simple tasks. Why shoot a fly with a bazooka? In this vignette, we are going to show that sometimes the bazooka is actually so versatile that is works just as easily on a fly. In particular, we will investigate how the combination **GAM + Effect Derivative** can be used even in simple contexts to make your data analysis easier.
...
Great, we know there is a significant correlation between the two variables! :partying_face: But what we would like to know is **for every increase of 1 on *x*, how much does *y* increase?** In other words, what is the **slope** of the relationship?
...
So the parameters table shows us that the **effect of x** is `12.75`. For every increase of 1 on *x*, *y* increases by *12.75*. Congrats, we've answered our question!

# GAMs can be used for linear relationships too!

> A new player has entered the game.

You might have heard of **General Additive Models**, aka **GAMs**, that extend general linear models (GLMs) by enabling a natural modelization of non-linear relationships. But we can also use them **even if it's linear**, or in general if we are not sure about the exact shape of the relationship: a GAM usually penalizes wiggly patterns, so if the relationship is linear, it will approximate it nicely.

GAMs can be fitted using the `mgcv` package, and the only change is that we need to specify as **smooth** term (`s()`) for the variable that we want to estimate the (non-necessarily linear) relationship for.

```{r}
model_gam <- mgcv::gam(y ~ s(x), data = data)

plot(modelbased::estimate_relation(model_gam), line = list(color = "red")) 

parameters::parameters(model_gam)
```


Wow, the GAM-based modeled relationship is near-exactly the same as that of the GLM! **GAMs are powerful** :sunglasses:
...
> Owww, so this is problematic considering our question about the effect of *x* on *y* :confused:
...
We know that the slope is `12.75` (from our parameters analysis). Does it change accross the course of the relationship? No, because it is a straight line, so the slope is constant. If the slope is constant, then the derivative should be... a constant line too, right? Let's verify this.
...
The plot shows a straight horizontal line at 12.75, with a fixed confidence interval (the same as in the parameter table). That is expected, by definition, a linaer model models a straight line with a fixed slope.
...
plot(deriv, line = list(color = "red")) +
...
summary(deriv)  # TODO: fix this to group by sign
```

If you know a bit theory about derivatives, you won't be suprised to find out that the derivative of a 2nd order polynomial (`x + x^2`) is actually a linear line. We can see conclude from this plot that the slope is (**significantly**, because the confidence interval does not cover 0) negative until 0, and then becomes positive. 0 corresponds indeed to the point of inversion of the curve.
...
p1 <- plot(modelbased::estimate_relation(model_gam2, length = 30), line = list(color = "blue")) 

deriv <- modelbased::estimate_slopes(model_poly, trend = "x", at = "x")

p2 <- plot(deriv, line = list(color = "blue")) +
  geom_hline(yintercept = 0, linetype = "dashed") 
