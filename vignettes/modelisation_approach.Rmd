...
1. We don't know whether the model lying underlying the test (the *t*-test, the
   ...
   turn to adjust the confidence in the results.
...
```{r message=FALSE, warning=FALSE}
posthoc <- emmeans(results, ~ Condition * Category) %>% 
  pairs()
parameters(posthoc)
```
...
We can use `geom_smooth()`, which can fit non-linear relationships in an emprical way, to give us an idea of the shape of the relationships.
