...
- ... are usually different for both linear and generalized linear models, when the data is "imbalanced" and "average" predictions are requested (i.e., `estimate = "average"` or `estimate = "population"`); by _imbalanced_ we mean not equally distributed levels.

::: custom_note
<p>**Note:** If you request unit-level predictions - that is, predictions specific to the individual levels of the random effects (achieved by including random effect variables in `by`) - conditional and marginal predictions will also differ in linear mixed models. We will not delve into unit-level predictions in this vignette.</p>
:::
