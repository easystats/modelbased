...
> It is also possible to define different sets of predictors for each class. Simply specify a formula for each parameter of the mixture model. For example, if we want to predict the mean QoL for each class differently, we can use `mu1` and `mu2` as separate formulas. We could also model the probability of class membership (`theta`) as a function of predictors. We will not cover this in detail here, but there an example code that illustrates how to do this and a summary table of common scenarios are provided at the end of this vignette.
