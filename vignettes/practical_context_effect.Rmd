...
> **Note:** Throughout this example, we use `display(format = "tt")` to display table output in markdown format, using the **tinytable** package as the backend. See [this vignette](https://easystats.github.io/insight/articles/display.html#displaying-tables-with-display) for more details on output displays.

> **Note:** Because within-effects can vary across individuals, it is generally best practice to model them as random slopes. However, to keep this example simple, we have only included `time` as a random slope.
