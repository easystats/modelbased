...
data <- data_modify(data, error = ifelse(as.character(response) != as.character(stim_cat), 1, 0))
```
...
```{r}
data_sub <- data_rt |>
  summarize(rt = mean(rt), .by = c("id", "condition")) |>
  pivot_wider(names_from = "condition", values_from = "rt", names_prefix = "empirical_") |>
  mutate(empirical_speed = empirical_accuracy - empirical_speed)
data_sub
```
...
```{r}
data_sub <- datawizard::data_join(data_sub, summary(reshaped), by = "id")
data_sub
```
...
The key idea is to compare the inter-individual variability in the random effects to their intra-individual variability in the data. 

For that, we first need to compute the variability (SD) of the point-estimates across participants.

```{r}
reliability <- random |>
  summarize(sd_between = sd(Coefficient), .by = "Parameter")
reliability
```
...
```{r}
reliability <- random |>
  summarize(sd_within = mean(SE), .by = "Parameter") |>
  datawizard::data_join(reliability)
reliability
```
...
```{r}
reliability |>
  mutate(reliability = sd_between / sd_within)
```
