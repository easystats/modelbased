...
We first look at the simple effect of `treatment`, which is about `r round(as.vector(glmmTMB::fixef(tmp)$cond["treatment1"]), 2)`.
...
Now we run the same model again, including the weights. We see that the treatment effect, after weighting, is about `r round(as.vector(glmmTMB::fixef(tmp)$cond["treatment1"]), 2)` points, i.e. the treatment on average results in a QoL score that is `r round(as.vector(glmmTMB::fixef(tmp)$cond["treatment1"]), 2)` points higher in comparison to the non-treated group.
...
est2 <- estimate_contrasts(tmp, "treatment", estimate = "population")
```
...
While the contrasts based solely on IPW are slightly biased with an estimated difference of `r round(est1$Difference, 2)`, the contrasts derived from the same model using g-computation are close to the effect we found before, with an estimated difference of `r round(est2$Difference, 2)`.
...
```{r}
# complex model, with IPW *and* G-computation (double-robust) is accurate!
estimate_contrasts(m4, "treatment", vcov = "HC0", estimate = "population")
```
