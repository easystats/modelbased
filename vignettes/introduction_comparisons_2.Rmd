...
Comparisons are extremly flexible. For instance, it is possible to calculate the marginal effects for the predictor `score`, or the marginal effects for `score` at each level of the treatment, `tx_ori`. We can do this by using `estimate_slopes()`.

```{r}
# marginal effect for "score"
estimate_slopes(m2, "score")

# marginal effect for "score" at levels of "tx_ori"
estimate_slopes(m2, "score", by = "tx_ori")
```
...
In essence, in the next example, the marginal effect (slope) of `score` within each level of `tx_ori` is calculated, and then two specific weighted combinations of these slopes as defined by the `cond_tx` matrix are computed. The first combination isolates and scales the slope from the first level of `tx_ori`, while the second creates a weighted sum of the slopes from the second and third levels.
...
# marginal effect for "score" at average effect of "treatment" (short + long)
# and "no treatment"
estimate_slopes(m2, "score", by = "tx_ori", comparison = cond_tx)
```
...
The above example can be easily expaned to three-way interactions.
