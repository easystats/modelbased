---
title: "Comparisons Of Slopes, Floodlight And Spotlight Analysis (Johnson-Neyman Intervals)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Comparisons Of Slopes, Floodlight And Spotlight Analysis (Johnson-Neyman Intervals)}
...
dat1 <- data.frame(
  x = dat$Sepal.Length[c(13, 28)],
  y = dat$Mean[c(13, 28)],
  group_col = "setosa",
  stringsAsFactors = FALSE
)
dat2 <- data.frame(
  x = dat$Sepal.Length[c(8, 23)],
  y = dat$Mean[c(8, 23)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
dat3 <- data.frame(
  x = dat$Sepal.Length[c(39, 54)],
  y = dat$Mean[c(39, 54)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
...
Since we don't want to do pairwise comparisons, we still use `estimate_slopes()` to test whether the linear trend (by groups) is signiciant or not. In this case, when interaction terms are included, the linear trend (_slope_) for our numeric focal predictor, `Sepal.Length`, is tested for each level of `Species`.
...
```{r echo=FALSE}
ht5 <- estimate_contrasts(m, "Sepal.Length", by = "Species")
```

The linear trend of `Sepal.Length` within `setosa` is significantly different from the linear trend of `versicolor` and also from `virginica`. The difference of slopes between `virginica` and `versicolor` is not statistically significant (p = `r round(ht5$p[3], 3)`).
...
dat <- as.data.frame(pred)
dat1 <- data.frame(
  x = dat$Sepal.Length[c(10, 16)],
  y = dat$Mean[c(10, 16)],
  group_col = "setosa",
  stringsAsFactors = FALSE
)
dat2 <- data.frame(
  x = dat$Sepal.Length[c(11, 17)],
  y = dat$Mean[c(11, 17)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
dat3 <- data.frame(
  x = dat$Sepal.Length[c(40, 46)],
  y = dat$Mean[c(40, 46)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
dat4 <- data.frame(
  x = dat$Sepal.Length[c(42, 48)],
  y = dat$Mean[c(42, 48)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
dat5 <- data.frame(
  x = dat$Sepal.Length[c(13, 14)],
  y = dat$Mean[c(13, 14)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
dat6 <- data.frame(
  x = dat$Sepal.Length[c(43, 45)],
  y = dat$Mean[c(43, 45)],
  group_col = "versicolor",
  stringsAsFactors = FALSE
)
...
```{r echo=FALSE}
ht6 <- estimate_slopes(m, "Sepal.Length", by = "Species")
```
...
```{r echo=FALSE}
ht7 <- estimate_contrasts(m, "Sepal.Length", by = "Species", comparison = "(b1-b2)=(b1-b3)"
)

```

The difference between the two differences is `r round(ht7$Difference[1], 2)` and not statistically significant (p = `r round(ht7$p[1], 3)`).
...
```{r}
estimate_contrasts(m, "Petal.Length", by = "Petal.Width=[sd]")
```
...
It's difficult to say at which values from `Illiteracy`, the association between `Murder` and `Income` might be statistically signifiant. We still can use `estimate_slopes()`:
...
However, this test can be simplified using the `summary()` function. This will show us in detail at which values for `Illiteracy` the interaction term is statisticall significant, and whether the association between `Murder` and the outcome is positive or negative.
...
[Go to next vignette: **Contrasts and Comparisons for Generalized Linear Models**](https://easystats.github.io/modelbased/articles/introduction_comparisons_3.html)
