...
For our quantity of interest, the contrast between episode `r paste0(ht1$Level1[1], "-", ht1$Leve21[1])`, we see the value `r round(ht1$Difference[1], 2)`, which is exactly the difference between the predicted outcome for `episode = 1` (`r round(my_predictions$Mean[1], 2)`) and `episode = 2` (`r round(my_predictions$Mean[2], 2)`). The related p-value is `r round(ht1$p[1], 3)`, indicating that the difference between the predicted values of our outcome at these two levels of the factor _episode_ is indeed statistically significant.

We can also define "representative values" via the `contrast` or `by` arguments. For example, we could also specify the levels of `episode` directly, to simplify the output:
...
```{r}
estimate_mean(model2, by = c("episode", "grp"))
```
