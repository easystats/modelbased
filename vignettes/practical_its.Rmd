---
title: "Interrupted Time Series Analysis with `modelbased`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Interrupted Time Series Analysis with modelbased}
...
```{r}
estimate_means(mod, at = c("time=c(199,200)", "event"))
```
...
```{r}
estimate_contrasts(mod, contrast = "event", at = "time=200")
```
...
```{r}
estimate_slopes(mod, trend = "time", at = "event")
```
...
```{r}
estimate_contrasts(mod, contrast = "time", at = "event")
```
