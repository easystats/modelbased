...
::: custom_note
<p>**Notes:**</p>
<p>The x-axis automatically adjusts its limits when categorical predictors are used (by setting `xlim` to `c(0.5, n + 0.5)`, the geoms are moved closer together, resulting in a more compact appearance). If appearance is too compact, specify different values for `xlim`, for instance, `xlim = c(1, n)` (where `n` is the number of unique categories).</p>
<p>When `facet`s are used, it might be useful to remove dodging, to harmonize the position of geoms across facets. To do so, set `dodge = 0`.</p>
:::
