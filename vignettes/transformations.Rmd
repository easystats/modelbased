...
Normalization is, in fact, a special case of **Rescaling**. Any variable can be rescaled to a new range. In R, it can be achieved via the `change_scale()` function.

```{r message=FALSE, warning=FALSE}
datawizard::change_scale(c(4, 5, 2, 4, 42), to = c(-1, 1))
```
...
check_transformation(data, "x_reciprocical", color = "green")
```
