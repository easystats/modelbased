...
`m`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Width`` ``~`` ``Sepal.Length`` ``+`` ``Species``, data ``=`` ``iris``)`\
[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.html)`(``m``)`\
...
\
[`estimate_means`](https://easystats.github.io/modelbased/reference/estimate_means.md)`(``m``, ``"Sepal.Length=c(4,5,6,7)"``)`\
...
\
[`estimate_slopes`](https://easystats.github.io/modelbased/reference/estimate_slopes.md)`(``m``, ``"Sepal.Length"``)`\
...
\
`m`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Width`` ``~`` ``Sepal.Length`` ``*`` ``Species``, data ``=`` ``iris``)`\
`pred`` ``<-`` `[`estimate_means`](https://easystats.github.io/modelbased/reference/estimate_means.md)`(``m``, `[`c`](https://rdrr.io/r/base/c.html)`(``"Sepal.Length"``, ``"Species"``)``)`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``pred``)`
...
\
[`estimate_slopes`](https://easystats.github.io/modelbased/reference/estimate_slopes.md)`(``m``, ``"Sepal.Length"``, by ``=`` ``"Species"``)`\
`#> Estimated Marginal Effects`\
`#> `\
...
\
[`estimate_contrasts`](https://easystats.github.io/modelbased/reference/estimate_contrasts.md)`(``m``, ``"Sepal.Length"``, by ``=`` ``"Species"``)`\
...
\
`m`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Width`` ``~`` ``Petal.Length`` ``*`` ``Petal.Width``, data ``=`` ``iris``)`\
`pred`` ``<-`` `[`estimate_means`](https://easystats.github.io/modelbased/reference/estimate_means.md)`(``m``, `[`c`](https://rdrr.io/r/base/c.html)`(``"Petal.Length"``, ``"Petal.Width=[sd]"``)``)`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``pred``)`
...
\
[`estimate_slopes`](https://easystats.github.io/modelbased/reference/estimate_slopes.md)`(``m``, ``"Petal.Length"``, by ``=`` ``"Petal.Width=[sd]"``)`\
`#> Estimated Marginal Effects`\
`#> `\
...
\
[`estimate_contrasts`](https://easystats.github.io/modelbased/reference/estimate_contrasts.md)`(``m``, ``"Petal.Length"``, by ``=`` ``"Petal.Width=[sd]"``, digits ``=`` ``1``)`\
`#> Marginal Contrasts Analysis`\
`#> `\
...
\
`states`` ``<-`` `[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``state.x77``)`\
`states``$``HSGrad`` ``<-`` ``states``$``` `HS Grad` ``\
`m_mod`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Income`` ``~`` ``HSGrad`` ``+`` ``Murder`` ``*`` ``Illiteracy``, data ``=`` ``states``)`\
\
`pr`` ``<-`` `[`estimate_means`](https://easystats.github.io/modelbased/reference/estimate_means.md)`(``m_mod``, `[`c`](https://rdrr.io/r/base/c.html)`(``"Murder"``, ``"Illiteracy"``)``)`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``pr``)`
...
\
[`estimate_slopes`](https://easystats.github.io/modelbased/reference/estimate_slopes.md)`(``m_mod``, ``"Murder"``, by ``=`` ``"Illiteracy"``)`\
`#> Estimated Marginal Effects`\
`#> `\
`#> Illiteracy |   Slope |    SE |            95% CI | t(45) |     p`\
`#> ----------------------------------------------------------------`\
`#> 0.50       |   82.08 | 31.47 | [  18.70, 145.47] |  2.61 | 0.012`\
`#> 0.76       |   57.24 | 26.92 | [   3.03, 111.45] |  2.13 | 0.039`\
`#> 1.01       |   32.49 | 25.04 | [ -17.95,  82.94] |  1.30 | 0.201`\
`#> 1.27       |    7.65 | 26.42 | [ -45.55,  60.86] |  0.29 | 0.773`\
`#> 1.52       |  -17.09 | 30.59 | [ -78.70,  44.52] | -0.56 | 0.579`\
`#> 1.78       |  -41.93 | 36.64 | [-115.74,  31.87] | -1.14 | 0.259`\
`#> 2.03       |  -66.68 | 43.77 | [-154.83,  21.48] | -1.52 | 0.135`\
`#> 2.29       |  -91.52 | 51.57 | [-195.38,  12.34] | -1.77 | 0.083`\
`#> 2.54       | -116.27 | 59.73 | [-236.56,   4.03] | -1.95 | 0.058`\
`#> 2.80       | -141.11 | 68.17 | [-278.41,  -3.81] | -2.07 | 0.044`\
`#> `\
...
`slopes`` ``<-`` `[`estimate_slopes`](https://easystats.github.io/modelbased/reference/estimate_slopes.md)`(``m_mod``, ``"Murder"``, by ``=`` ``"Illiteracy"``, length ``=`` ``200``)`\
[`summary`](https://rdrr.io/r/base/summary.html)`(``slopes``)`\
...
\
`pr`` ``<-`` `[`estimate_means`](https://easystats.github.io/modelbased/reference/estimate_means.md)`(``m_mod``, `[`c`](https://rdrr.io/r/base/c.html)`(``"Murder"``, ``"Illiteracy=c(0.7,1.5,2.8)"``)``)`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``pr``)`` ``+`` ``ggplot2``::`[`facet_wrap`](https://ggplot2.tidyverse.org/reference/facet_wrap.html)`(``~``Illiteracy``)`
